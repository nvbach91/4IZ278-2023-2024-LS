<script lang="ts">
	...
	import type { Client as ClientType, Generated } from '$types/user';
	...
	{:else}
		...
		<div class="mt-4 grid max-w-full grid-cols-[repeat(auto-fill,_minmax(256px,_1fr))] gap-4">
			{#each clients as client}
				...
				<Client {client}>
					{#if $user?.role === 'admin' && amount}
						<p>
							Total payments: {amount.toFixed(2)} CZK
						</p>
						<p>
							Total fee: {(amount * (client.fee / 100)).toFixed(2)} CZK
						</p>
					{/if}
				</Client>
			{/each}
							...
							<FormItem>
								<Label for="fee" class="flex items-center gap-2">
									...
									<Tooltip.Root>
										...
										<Tooltip.Content>
											The fee is the percentage of the total sale that the client will pay to QR
											Café. If you wish to change the fee, please contact an administrator.
										</Tooltip.Content>
									</Tooltip.Root>
								</Label>
								<Input type="number" id="fee" name="fee" bind:value={newFee} disabled />
							</FormItem>
							<FormItem row>
