...
if (isset($_SESSION['admin_book']['book_id'])) {
  unset($_SESSION['admin_book']);
}
...
if (!empty($_POST)) {
  ...
  if (count($errors) == 0) {
    $successMessage = "Kniha vytvořena!";
    $booksDB->createBook($name, $author, $price, $discount, $units, $publish_date, $language, $image, $description, $pages, $rating, $publisher_id, $genre_id, $world_id);
    unset($_SESSION['admin_book']);
  }
}
