...
class ReservationsDB extends Database
{
    ...
    public function findByDateAndCarOld($date, $car){
        ...
        left join (SELECT association.reservation_id, association.car_id, reservations.date, reservations.time, users.name, users.email, users.hotel FROM association
                ...
                ORDER BY car_id ASC, time ASC) as asd on asd.time = time_blocks.time_id", [":date" => $date, ":car" => $car]);
        return $results;
    }
    ...
    public function findByDateAndCar($date, $car){
        ...
        left join (SELECT association.reservation_id, association.car_id, reservations.date, reservations.time, users.name, users.email, users.hotel FROM association
                ...
                LEFT JOIN hotels on hotel = hotels.hotel_id", [":date" => $date, ":car" => $car]);
        return $results;
    }

    public function isReservation($user_id, $date, $time){
        $results = $this->runQuery("SELECT * from reservations where booked_by like :user_id and date like :date and time like :time", [":user_id" => $user_id, ":date"=> $date, ":time"=> $time]);
        if (count($results) > 0){
    ...
    {
        $results = $this->runQuery("INSERT INTO reservations (booked_by, date, time) values (:user_id, :date, :time) ", [":user_id" => $user_id, ":date"=> $date, ":time"=> $time]);
        return $results;
    }
}