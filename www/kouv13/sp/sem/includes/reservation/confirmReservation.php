...
$array = $actions->sortArray(json_decode($_POST['ids'], true));

$monday = htmlspecialchars($_POST['monday']);

$array = $actions->correctDates($array, $monday);
...
if (isset($array) && isset($monday) && isset($idField) && isset($idSport) && isset($price)) {
    ...
    foreach ($array as $element) {
        $isFull = $reservation->isFull($element->time, $element->date, $idField);
        var_dump($isFull);
        if (date('Y-m-d') <= $element->date && $element->date <= $two_weeks_sunday && empty($isFull)) {
            $id = $reservation->addReservation($idUser, $idField, $idSport, $element->date, $element->time, $element->day, $price, $status);
    ...
    if (empty($reservationIds)) {
        ...
        if (isset($_SESSION['admin'])) {
            ...
            foreach ($reservationIds as $id) {
                $res = $reservation->getMyReservation($_SESSION['iduser'], htmlspecialchars($id));
            ...
            $html = $html1 . $actions->getCheckoutReservations($reservations) . $html2;
            $actions->sendCheckout($html);
