...
foreach ($allSports as $s) {
    $is = false;
    if (in_array($s->sport_id, $fieldSport_ids, true)) {
        $is = true;
    }
    ?>
    ...
    <div class="col-2 sport-editing border rounded-2 p-3 <?php if ($is == true) {
        echo 'is-sport';
    } ?>" role="button" onclick="editSports('<?php echo $_GET['idField']; ?>', '<?php echo $s->sport_id; ?>');"><strong
            class="fw-semibold"><?php echo $s->name; ?></strong></div>
