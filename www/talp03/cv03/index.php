...
if (!empty($_POST)) {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        array_push($errors, "$email is not valid email format!");
    }
    if (!filter_var($avatar, FILTER_VALIDATE_URL)) {
        array_push($errors, "$avatar is not valid URL!");
    }
    if (!preg_match('/^(\+\d{1,3}\s?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{3}$/', $phone)) {
        array_push($errors, "$phone is not a valid phone number. e.g. +420 345 676 876");
    }
    if (strlen($deckName) < 3) {
        array_push($errors, "$deckName is not valid! Deck name must have 3 or more characters.");
    }
